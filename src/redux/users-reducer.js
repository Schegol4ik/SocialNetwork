...
const TOGGLE_IS_FOLLOWING = 'TOGGLE-IS-PROGRESS'
...
export const follow = (userId) => {
    return (dispatch) => {
        ...
        UserAPI.controlFollowed.getFollow(userId).then(response => {
...
export const unfollow = (userId) => {
    return (dispatch) => {
        dispatch(toggleIsFollowing(true, userId));
        UserAPI.controlFollowed.getUnfollow(userId).then(response => {
            if (response.data.resultCode === 0) {
